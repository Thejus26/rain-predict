...
1. **Stop 2 Deep Sleep Mode Current**: $I_{\text{sleep}} < \mathbf{5.0\,\mu\text{A}}$ (target baseline: $2.8\text{--}3.0\,\mu\text{A}$ at $3.3\text{V}$ with full 64 KB SRAM retention and RTC wakeup timer running).
2. **Active Measurement Execution Window**: Total CPU active execution time $t_{\text{active}} \le \mathbf{1.20\text{ seconds}}$ ($1200\text{ ms}$) per cycle.
3. **Active Cycle Average Operating Current**: $I_{\text{active, avg}} < \mathbf{25.0\text{ mA}}$ across all sensor sampling, algorithmic filtering, NVM logging, and Sub-GHz LoRaWAN transmission phases.
...
| :--- | :---: | :---: | :---: | :---: | :---: | :--- |
| **`STATE_WAKE`** | $0.5\text{ ms}$ | $4.5\text{ mA}$ | $4.8\text{ mA}$ | $0.000625\text{ \mu Ah}$ | $0.00743\text{ mJ}$ | Clock restored from Stop 2 to MSI 48 MHz; IWDG kicked. |
| **`STATE_POWER_ON`** | $20.0\text{ ms}$ | $1.2\text{ mA}$ | $2.5\text{ mA}$ | $0.006667\text{ \mu Ah}$ | $0.07920\text{ mJ}$ | `PA4` load switch enabled; $20\text{ms}$ RC stabilization delay strictly observed. |
| **`STATE_SAMPLE`** | $65.0\text{ ms}$ | $7.8\text{ mA}$ | $9.5\text{ mA}$ | $0.140833\text{ \mu Ah}$ | $1.67310\text{ mJ}$ | BME280 forced burst, OPT3001 conversion, battery ADC sampling. |
| **`STATE_FILTER`** | $4.0\text{ ms}$ | $4.5\text{ mA}$ | $4.8\text{ mA}$ | $0.005000\text{ \mu Ah}$ | $0.05940\text{ mJ}$ | Magnus-Tetens dew point, moving-average filters, trend derivatives. |
| **`STATE_PREDICT`** | $5.0\text{ ms}$ | $4.5\text{ mA}$ | $4.8\text{ mA}$ | $0.006250\text{ \mu Ah}$ | $0.07425\text{ mJ}$ | Zambretti 26-rule evaluation, Composite Precipitation Index ($CPI$). |
| **`STATE_TRANSMIT`** | $60.0\text{ ms}$ | $32.0\text{ mA}$ | $36.0\text{ mA}$ | $0.533333\text{ \mu Ah}$ | $6.33600\text{ mJ}$ | LoRaWAN Sub-GHz TX (+14 dBm) and on-chip Flash ring buffer push. |
| **`STATE_ALERT`** | $10.0\text{ ms}$ | $2.5\text{ mA}$ | $5.0\text{ mA}$ | $0.006944\text{ \mu Ah}$ | $0.08250\text{ mJ}$ | Visual status LED flash, buzzer/siren dispatch logic. |
| **`STATE_SLEEP`** | $2.0\text{ ms}$ | $0.8\text{ mA}$ | $1.0\text{ mA}$ | $0.000444\text{ \mu Ah}$ | $0.00528\text{ mJ}$ | `PA4` rail disabled, GPIOs conditioned to analog, Stop 2 entry. |
| **Active Cycle Subtotal** | **$\mathbf{166.5\text{ ms}}$** | $\mathbf{I_{\text{active, avg}} = 15.13\text{ mA}}$ | $\mathbf{36.0\text{ mA}}$ | $\mathbf{0.700096\text{ \mu Ah}}$ | $\mathbf{8.31716\text{ mJ}}$ | **Active window well below $1.20\text{s}$ limit and $25\text{mA}$ current ceiling.** |
| **`STATE_STOP2_STANDBY`** | $899.83\text{ s}$ | $3.0\,\mu\text{A}$ ($0.003\text{ mA}$) | $3.2\,\mu\text{A}$ | $0.749861\text{ \mu Ah}$ | $8.90835\text{ mJ}$ | Deep sleep ($15\text{-min}$ nominal minus $0.166\text{s}$ active duration). |
| **Total 15-min Mission** | **$\mathbf{900.00\text{ s}}$** | $\mathbf{I_{\text{mission, avg}} = 5.80\,\mu\text{A}}$ | — | $\mathbf{1.450\times 10^{-3}\text{ mAh}}$ | $\mathbf{17.225\text{ mJ}}$ | **Ultra-low overall current ensures multi-year battery autonomy.** |
...
| **TC-PWR-12** | Total Single-Cycle Charge ($Q_{\text{cycle}}$) | Complete 15-minute nominal measurement cycle | **$Q_{\text{cycle}} \le 1.60\times 10^{-3}\text{ mAh}$** ($1.60\text{ \mu Ah}$) |
...
2. **Ceiling Verification Compliance**:
   - Stop 2 sleep current is confirmed at $< \mathbf{5.0\,\mu\text{A}}$ (nominal $\le 3.0\,\mu\text{A}$).
   - Active cycle duration is confirmed at $\le \mathbf{1.20\text{ seconds}}$ (nominal $\approx 166.5\text{ ms}$).
   - Active cycle average current is confirmed at $< \mathbf{25.0\text{ mA}}$ (nominal $\approx 15.13\text{ mA}$).
3. **Leakage Suppression Certified**: Pre-sleep analog pin isolation reduces unpowered sensor rail leakage to $< \mathbf{50\text{ nA}}$.
