...
3. **Complex Bit-Split Unpacking for `dig_H4` and `dig_H5`**:
   - The 12-bit signed humidity coefficients `dig_H4` and `dig_H5` share register `0xE5`:
     $$\text{dig\_H4} = (\text{int16\_t})\left( ((\text{int8\_t})\text{reg}[0\text{xE4}] \ll 4) \mid (\text{reg}[0\text{xE5}] \ \& \ 0\text{x0F}) \right)$$
     $$\text{dig\_H5} = (\text{int16\_t})\left( ((\text{int8\_t})\text{reg}[0\text{xE6}] \ll 4) \mid ((\text{reg}[0\text{xE5}] \gg 4) \ \& \ 0\text{x0F}) \right)$$
