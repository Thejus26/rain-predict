---
name: feature
description: Manage current firmware feature workflow - start, review, explain or complete
...
- `## Notes` - Hardware constraints, register maps, or power specs
...
2. Update `current-feature.md`:
   - Update H1 heading to include feature name (e.g., `# Current Feature: BME280 Driver`)
...
4. Create and checkout the feature branch (derive name from H1 heading, e.g., `feature/bme280-driver`)
...
3. Check for:
   ...
   - ⚡ Low-power hygiene and non-blocking I/O
   - ⚠️ Code quality issues, status code handling, or bugs
...
If no action provided, explain the available options: `load`, `start`, `review`, `explain`, `complete`